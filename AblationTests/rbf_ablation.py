...
Jacob Maurer
9/4/2026
Purpose: Hyperparameter tune for radial basis function kans
"""
...
DEVICE = "cuda"
class ClassAblatRBFKAN(nn.Module):
    def __init__(self, num_centers, alpha):
        super().__init__()
        self.model = nn.Sequential(
            RBFKANLayer(4, 5, num_centers, alpha),
            RBFKANLayer(5, 4, num_centers, alpha)
        )
    def forward(self, x):
        return self.model(x)

class RegressAblatRBFKAN(nn.Module):
    def __init__(self, num_centers, alpha):
        super().__init__()
        self.model = nn.Sequential(
            RBFKANLayer(4, 11, num_centers, alpha),
            RBFKANLayer(11, 1, num_centers, alpha)
        )
    def forward(self, x):
        return self.model(x)    
...
def load_class_rbf():
    ...
    def objective(trial):
        train_settings = {
            ...
            "num_centers": trial.suggest_int("num_centers", 20, 100),
            "lr": trial.suggest_float("lr", 0.0001, 0.3),
        }
        class_model = ClassAblatRBFKAN(train_settings["num_centers"], train_settings["alpha"]).to(DEVICE)
...
def load_regress_rbf():
    ...
    def objective(trial):
        ...
        regress_model = RegressAblatRBFKAN(train_settings["num_centers"], train_settings["alpha"]).to(DEVICE)
